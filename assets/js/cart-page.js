...
(function () {
  ...
  function render() {
    ...
    if (summaryLine) summaryLine.textContent =
      totalQty === 1 ? '1 item in your cart' : `${totalQty} items in your cart`;
  ...
  function wireRows() {
    document.querySelectorAll('.cart-card').forEach(card => {
      ...
      card.querySelector('[data-remove]')?.addEventListener('click', () => window.RainFowCart.remove(id));
    });
  }
  ...
  function wireClear() {
    ...
    if (!btn) return;
    btn.addEventListener('click', () => {
      if (confirm('Remove all items from your cart?')) window.RainFowCart.clear();
    });
  }
  ...
  function init() {
    ...
    window.addEventListener('storage', e => { if (e.key === 'rainfow_cart') render(); });
  }
